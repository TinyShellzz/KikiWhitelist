...
public class BanlistMapper {
    ...
    public BanlistUser get_user_by_uuid(String mc_uuid){
        ...
        try {
            ...
            rs = stmt.executeQuery();
            if(rs.next()) {
                ret = new BanlistUser(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5));
            }
        } catch (SQLException e) {
            Bukkit.getConsoleSender().sendMessage(ChatColor.RED + e.getClass().getName() + ": " + e.getMessage());
        } finally {
    ...
    public void update_by_uuid(BanlistUser user){
        ...
        try {
            conn = Config.connect();
            stmt = conn.prepareStatement("UPDATE banlist SET user_name=?, display_name=?, unban_date=?, reason=? WHERE mc_uuid=?");
            stmt.setString(1, user.user_name);
            stmt.setString(2, user.display_name);
            stmt.setString(3, user.unban_date);
            stmt.setString(4, user.reason);
            stmt.setString(5, user.mc_uuid);
            stmt.executeUpdate();
            conn.commit();
        } catch (SQLException e) {
